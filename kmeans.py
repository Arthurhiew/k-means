...
def toyProblem():
    ...
    for _ in range(50):
        _, _, SSE = kMeansClustering(X, k=k, max_iters=max_iters, visualize=False)
        SSE_rand.append(SSE)
    ...
    plt.xlabel("SSE")
    plt.ylabel("# Runs")
    plt.title("Randomness in Clustering")
    plt.show()
...
def imageProblem():
    ...
    k = 3
    centroids, assignments, SSE = kMeansClustering(img_feats, k=k, max_iters=30, min_size=0)
    print("SSE = ", SSE[-1])
    # Visualize Clusters
...
def initalizeCentroids(dataset, k):
    ...
    centroids = dataset[np.random.choice(np.arange(len(dataset)), k), :]
    return centroids
...
def computeAssignments(dataset, centroids):
    assignments = np.argmin(np.linalg.norm(np.subtract(dataset, centroids[:, np.newaxis]), axis=2), axis=0)
    return assignments
...
def updateCentroids(dataset, centroids, assignments):
    count = np.zeros(centroids.shape[0])
    for k in range(centroids.shape[0]):

        count[k] = len(dataset[assignments == k])
        if count[k]:
            centroids[k] = np.mean(dataset[assignments == k], axis=0)
    # print(count)
...
def calculateSSE(dataset, centroids, assignments):
    sse = np.sum(np.linalg.norm(np.subtract(dataset, centroids[assignments])))
    return sse
